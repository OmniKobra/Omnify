...
class FeesProvider with ChangeNotifier {
  ...
  Future<List<Decimal>> getAndSetPaymentFees(
    Chain c,
    Web3Client client,
  ) async {
    final alias = Aliases.getAlias(c);
    ...
    return [Utils.dec(c, feePerPayment), Utils.dec(c, feePerInstallment)];
  }

  Future<List<Decimal>> getAndSetTrustFees(
    Chain c,
    Web3Client client,
  ) async {
    final alias = Aliases.getAlias(c);
    ...
    return [Utils.dec(c, depositFee), Utils.dec(c, benefFee)];
  }

  Future<Decimal> getAndSetBridgeFees(
    Chain c,
    Web3Client client,
  ) async {
    final alias = Aliases.getAlias(c);
    ...
    return Utils.dec(c, bridgeFee);
  }

  Future<Decimal> getAndSetEscrowFees(
    Chain c,
    Web3Client client,
  ) async {
    final alias = Aliases.getAlias(c);
